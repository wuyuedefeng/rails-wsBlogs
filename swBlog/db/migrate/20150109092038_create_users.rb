class CreateUsers < ActiveRecord::Migration
  def change
    create_table :users do |t|
      ...
      t.string    :crypted_password, limit: 150,    :null => false                # 可选，但强烈建议加上。  
      t.string    :password_salt, limit: 50,       :null => false                # 可选，用于辅助密码验证的字段(不可逆算法进行密码验证时所需) 就是一个密钥,用来给Password加密  
      t.string    :persistence_token, limit: 150,   :null => false                # 必选   Authlogic内部使用的。它存储在你的cookies和session里，用于持久化当前用户的会话。这种方式比单纯的保存user id要安全得多
      t.string    :single_access_token, limit: 50, :null => false                # 可选，详见Authlogic::Session::Params  用于私有的API访问
      t.string    :perishable_token, limit: 50,    :null => false                # 可选，详见Authlogic::Session::Perishability  重置密码时使用
      ...
      t.string    :current_login_ip, limit: 30                                   # 可选，详见Authlogic::Session::MagicColumns  
      t.string    :last_login_ip, limit: 30                                      # 可选，详见Authlogic::Session::MagicColumns  
      # ws
